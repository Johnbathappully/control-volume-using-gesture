...
def apply_augmentations(image, boxes, labels, aug):
    ...
    if 'angle' in transformed:
        for i in range(len(boxes)):
            boxes[i] = rotate_box(boxes[i], image.shape[0], image.shape[1], transformed['angle'])
    return image, boxes
...
def rotate_box(box, image_height, image_width, angle):
    x1, y1, x2, y2 = box
    cx, cy = (x1 + x2) / 2, (y1 + y2) / 2
    radians = math.radians(angle)
    cosine = math.cos(radians)
    sine = math.sin(radians)
    # Translate center of bounding box to origin
    x1 -= cx
    y1 -= cy
    x2 -= cx
    y2 -= cy
    # Rotate bounding box coordinates
    x1_new = x1 * cosine - y1 * sine
    y1_new = x1 * sine + y1 * cosine
    x2_new = x2 * cosine - y2 * sine
    y2_new = x2 * sine + y2 * cosine
    # Translate bounding box back to original location
    x1_new += cx
    y1_new += cy
    x2_new += cx
    y2_new += cy
    # Clip bounding box coordinates to image size
    x1_new = max(0, min(x1_new, image_width-1))
    y1_new = max(0, min(y1_new, image_height-1))
    x2_new = max(0, min(x2_new, image_width-1))
    y2_new = max(0, min(y2_new, image_height-1))
...
def process_batch(input_images_folder, input_xml_folder, output_images_folder, output_xml_folder):
    ...
    for img_path in image_files:
        ...
        for i in range(10):
            ...
            new_xml_tree = update_xml(tree, new_boxes)
